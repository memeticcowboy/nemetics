---
term: Distinction
category: philosophy
source: NEMAtic Framework, Bateson (information as difference)
status: canonical
elemental_mapping:
  air: 0.90
  metal: 0.50
siml_entry: https://github.com/memeticcowboy/nema-swarm/tree/main/SIML/terms/M007_Distinction
---

# Distinction

**Dynamic differentiation without fixed substance or forced unity.**

σ-operator: cuts that enable, not scar tissue. Maintains ε (uncertainty) in the cut. Revisable: can be re-cut without rupture. Not separation but relational boundary.

**Principle:** "The cut that connects."

**Diagnostic:** Does the distinction open possibility or foreclose it?

**Related:** Multiplicity, ✶-State, Permeability

**SIML Entry:** [M007 Distinction](https://github.com/memeticcowboy/nema-swarm/tree/main/SIML/terms/M007_Distinction) — Full elemental analysis, nemetic string, daemon correspondence
